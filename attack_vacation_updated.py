...
leader_email = input("enter your account's email: ")
leader_password = input("enter password: ")
login_data = {"email": leader_email, "password": leader_password, "rem": "on", "reg": "התחברות >>"}
spell_data = {"res_hour_mad": input("enter shield time: "), "magic_res_defense": "הפעל את קסם מגן המשאבים"}
...
def login(p=0):
    ...
    while True:
        try:
            ...
            if response.status_code != 200:
                switch_proxy(s)
                #sync_print(response.status_code)
            else:
                break
        except:
            continue
    ...
    while True:
        ...
        for ids in x:
            if ids not in all_ids:
                new_ids.append(ids)
        sync_print(f'found {2*(len(new_ids)//2)} new targets')
        while len(new_ids) > 1:
            thread = threading.Thread(target=thread_op, args=(new_ids[:2], threshold, random.randint(0, len(ips)-1)))
            thread.start()
            all_ids.append(new_ids[0])
            all_ids.append(new_ids[1])
            new_ids = new_ids[2:]
...
while len(attack_links) > 0:
    thread = threading.Thread(target=thread_op, args=(attack_links[:2], threshold, random.randint(0, len(ips)-1)))
    thread.start()
    attack_links = attack_links[2:]

update_ids(all_ids, s)
